...
class TopicNode(Base):
    ...
    source_edges: Mapped[list["TopicEdge"]] = relationship("TopicEdge", foreign_keys="TopicEdge.source_node_id", back_populates="source_node", cascade="all, delete-orphan")
    target_edges: Mapped[list["TopicEdge"]] = relationship("TopicEdge", foreign_keys="TopicEdge.target_node_id", back_populates="target_node", cascade="all, delete-orphan")
...
class TopicEdge(Base):
    ...
    id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    user_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("users.id", ondelete="CASCADE"), nullable=False)
    source_node_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("topic_nodes.id", ondelete="CASCADE"), nullable=False)
    target_node_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("topic_nodes.id", ondelete="CASCADE"), nullable=False)
    relationship_type: Mapped[str] = mapped_column(String(50), default="related", nullable=False)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now(), nullable=False)

    source_node: Mapped["TopicNode"] = relationship("TopicNode", foreign_keys=[source_node_id], back_populates="source_edges")
    target_node: Mapped["TopicNode"] = relationship("TopicNode", foreign_keys=[target_node_id], back_populates="target_edges")
